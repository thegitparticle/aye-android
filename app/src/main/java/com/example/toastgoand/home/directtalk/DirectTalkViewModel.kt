...
class DirectTalkViewModel(repoDeets: UserDetailsRepo, private val repoRecos: DefaultRecosRepo): ViewModel() {
...
class DirectTalkViewModelFactory(
    private val repoDeets: UserDetailsRepo,
    private val repoRecos: DefaultRecosRepo
) : ViewModelProvider.Factory {
    override fun <T : ViewModel> create(modelClass: Class<T>): T {
        if (modelClass.isAssignableFrom(DirectTalkViewModel::class.java)) {
            @Suppress("UNCHECKED_CAST")
            return DirectTalkViewModel(repoDeets, repoRecos) as T
        }
